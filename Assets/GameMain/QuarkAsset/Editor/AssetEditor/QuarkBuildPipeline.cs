...
namespace Quark.Editor
{
    public class QuarkBuildPipeline
    {
...
#if UNITY_IOS
           QuarkUtility.LogInfo( "Please switch to Android environment" );
#else
        ...
        public static void BuildAndroidAssetBundle2( )
        {

#if UNITY_IOS
           QuarkUtility.LogInfo( "Please switch to Android environment" );
#else
            BuildAssetBundle( BuildTarget.Android, false, true );
#endif

        }

        [MenuItem( "Tools/Build IOS Assetbundle" )]
        public static void BuildIosAssetBundle2( )
        {

#if UNITY_ANDROID
            QuarkUtility.LogInfo( "Please switch to IOS environment" );
#else
            BuildAssetBundle( BuildTarget.iOS, false, true );
#endif
        ...
        public static string BuildAssetBundle( BuildTarget buildTarget, bool nameByHash = false, bool copyToStreamingAssets = false )
        {
            ...
            if ( dataset == null )
            {
                ...
                if ( assets.Length > 0 )
                {
                    dataset = AssetDatabase.LoadAssetAtPath<QuarkDataset>( AssetDatabase.GUIDToAssetPath( assets[ 0 ] ) );
                }
            ...
            if ( tabData == null )
            {
                try
                {
                    tabData = QuarkEditorUtility.GetData<QuarkAssetBundleTabData>( QuarkAssetBundleTab.AssetBundleTabDataFileName );
                }
                catch
                {
